...
export default class Grid {
  ...
  mousezoom(delta, event) {
    ...
    let processedEventDeltaY = Utils.get_deltaY(event); // Ensure we get a consistent deltaY

    if (Math.abs(event.deltaX) > 0) {
      this.trackpad = true;
      if (Math.abs(event.deltaX) >= Math.abs(processedEventDeltaY)) {
        delta *= 0.1;
      }
    ...
    let minZoomCheck = delta < 0 && this.data.length <= this.MIN_ZOOM;
    let maxZoomCheck = delta > 0 && this.data.length > this.MAX_ZOOM;
    ...
    if (event.originalEvent.ctrlKey || tl) {
      ...
      // Symmetrical zoom for default scroll
