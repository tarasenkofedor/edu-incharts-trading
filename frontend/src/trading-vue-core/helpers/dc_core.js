...
export default class DCCore extends DCEvents {
  ...
  init_data($root) {
    ...
    if (!("chart" in this.data) && this.data.ohlcv) {
      const newChartObject = { type: "Candles", data: this.data.ohlcv || [], settings: {} };
      if (this.tv) {
        this.tv.$set(this.data, "chart", newChartObject);
      } else {
        this.data.chart = newChartObject;
      }
    } else if (!("chart" in this.data)) {
        const newChartObject = { type: "Candles", data: [], settings: {} };
        if (this.tv) {
            this.tv.$set(this.data, "chart", newChartObject);
        } else {
            this.data.chart = newChartObject;
    ...
    if (this.data.chart && !this.data.chart.settings) {
        if (this.tv) {
    ...
    // Process main OHLCV data: copy from chart.data to this.ohlcv, then delete chart.data
    // Only process if this.ohlcv is not already populated (e.g., by a previous init_data call)
    if (!this.ohlcv || this.ohlcv.length === 0) {
        if (this.data.chart && this.data.chart.data) {
            this.ohlcv = this.data.chart.data;
            delete this.data.chart.data; // Delete the original array from chart object
        } else if (this.data.ohlcv) {
            // This case handles if `ohlcv` was directly provided (e.g. old data format or specific setup)
            this.ohlcv = this.data.ohlcv;
        } else {
            this.ohlcv = []; // Ensure ohlcv is an array
        }
    }

    // Remove top-level ohlcv if it exists, as it's now managed by this.ohlcv
    if (this.data.ohlcv) {
    ...
    // This part might need more careful handling if Dataset class uses this.tv
    if (this.data.datasets) {
        for (var ds of this.data.datasets) {
            if (!this.dss) this.dss = {};
            this.dss[ds.id] = new Dataset(this, ds); // Pass `this` (DCCore instance)
        }
    }
    this.update_ids(); // Call update_ids, which also needs to be resilient
  }
  ...
  update_ids() {
    if (!this.data.chart) this.data.chart = {type: 'Candles'}; // Ensure chart object exists
    ...
    if (!this.data.onchart) this.data.onchart = [];
    ...
    let grids = [{}];
    let gid = 0;
    if (!this.data.offchart) this.data.offchart = [];
